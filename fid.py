...
"""Script for calculating Frechet Inception Distance (FID). From edm-sqa/fid.py"""
...
def sqa(dest_path, ref_path):
    ...
    if dist.get_rank() == 0:
        with dnnlib.util.open_url(ref_path) as f:
            ref = np.load(f, allow_pickle=True) # sqa modified
            try:
                m1, s1 = ref["mu"][:], ref["sigma"][:]
            except IndexError:
                m1, s1 = ref.item()["mu"][:], ref.item()["sigma"][:]

    dist.print0(f'Loading dataset reference statistics from "{dest_path}"...')
    edm = None
    if dist.get_rank() == 0:
        with dnnlib.util.open_url(dest_path) as f:
            edm = np.load(f, allow_pickle=True) # sqa modified
            try:
                m2, s2 = edm["mu"][:], edm["sigma"][:]
            except IndexError:
                m2, s2 = edm.item()["mu"][:], edm.item()["sigma"][:]

    dist.print0('Calculating FID...')
    if dist.get_rank() == 0:
        fid = calculate_fid_from_inception_stats(m2, s2, m1, s1)
        print(f'{fid:g}')
    torch.distributed.barrier()
